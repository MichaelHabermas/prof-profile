...
- For Selected Work project cards, prefer a uniform grid with equal card dimensions; avoid side-attached imagery or decorative panels on cards.
...
- Tailwind CSS is loaded from the CDN for layout utilities; bespoke styling and components live in `css/main.css`. README points to `FEATURES.md` for feature-level implementation notes.
...
- Bubble chat hover, open, and close sounds pick randomly from short MP3 files in `assets/bubble-sounds/`.
